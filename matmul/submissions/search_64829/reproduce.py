"""Rebuild 64,829 from literal mixed-chunk loops with portable reference allocation."""
...
ROW_WIDTHS = (5, 5, 6)
COLUMN_WIDTHS = (6, 10)
MACRO_CHUNKS = (1, 1, 1, 2, 2, 2)
CAPTURE_THRESHOLD = 6
EXPECTED = {
    'captured_generator': (1173642, '093cd429879795f762f23ddd5d323952ea114d9072f70ec6d5db3698aa7188f8'),
    'dp_initial': (64929, '71cb9d284deebe670655e82679abb37f469415a52a39224c13d0d200c6462caa'),
    'pair2_seed0_no_ties': (64913, 'ae164e5c09855928f92e49c8ff142353a811d17b104a0f163b67eea6f9cec76f'),
    'pair5_seed11_ties': (64829, '0ee7a25b688c090150a8a4039cf394b6115fc6ab666827ac76471c0c83bb0c88'),
}
...
def reproduce(output=None, manifest=None):
    ...
    for name, rounds, seed, ties in [('pair2_seed0_no_ties', 2, 0, False),
        ...
        checkpoint(name, p)
    ir = core.emit(p)
    assert ir.encode() == (HERE.parent / 'best_64829.ir').read_bytes(), 'Replay differs from submission'
    ...
    result = {'score': 64829, 'sha256': EXPECTED['pair5_seed11_ties'][1],
    ...
    print('Portable generator replay is byte-identical to best_64829.ir.', flush=True)
    return result
